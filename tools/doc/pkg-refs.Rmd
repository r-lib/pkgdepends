...
If you did not explicitly specify any CRAN-like reposirories (e.g. with
...
- `param` a special reference to change how other references are downloaded
  or installed. See "Paraemters" below.
...
* For `github::` dependencies `r pak_or_pkgdepends()` assumes that the package
  ...
  `<package>=` prefix. E.g. `pins=rstudio/pins-r`. If you specify both the
...
- `nocache` will ignore the package cache. It will always download the
  ...
  to the package cache. It is supported by the `bioc::`, `cran::`, `github::`,
...
If you specify a local package in a dependency (ie. in `DESCRIPTION`), then
...
If you specify a package from an URL in a dependency (ie. in `DESCRIPTION`),
