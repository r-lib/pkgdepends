...
pkgplan_i_lp_dependencies <- function(lp) {
  ...
  depconds <- function(wh) {
    ...
    for (i in seq_len(nrow(deps))) {
      ...
      txt <- sprintf("%s depends on %s: %s", pkgs$ref[wh], depref,
                     cli::ansi_collapse(report))
...
format_cond <- function(x, cond) {
  if (cond$type == "dependency") {
    ...
    ref <- x$pkgs$ref[cond$vars]
    sprintf("`%s` needs an older R version: %s", ref, cond$note)
    ...
    ref <- x$pkgs$ref[cond$vars]
    sprintf("`%s` is direct, needs latest version", ref)
...
format.pkgplan_lp_problem <- function(x, ...) {
  ...
  if (length(x$conds)) {
    push(sprintf("+ constraints (%d}):", length(x$conds)))
