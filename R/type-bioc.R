...
#' @export
parse_remote.remote_specs_bioc <- NULL
#' @export
resolve_remote.remote_ref_bioc <- NULL
#' @export
download_remote.remote_resolution_bioc <- NULL
#' @export
satisfies_remote.remote_resolution_bioc <- NULL
...
local({
  ...
  matching_bioc_version <- function(r_version) {
    if (r_version >= "3.5") {
      "3.6"
  ...
  get_bioc_repos <- function(r_version) {
    bv <- matching_bioc_version(r_version)
    tmpl <- c(
  ...
  update_cache <- function(cache, rootdir, platforms, rversions) {
    cache; rootdir
    dirs <- get_all_package_dirs(platforms, rversions)

    bioc_repos <- lapply_with_names(rversions, get_bioc_repos)
  ...
  resolve_from_cache <- function(remote, config, bioccache) {
    files <- resolve_from_cache_files(remote, config, bioccache)
  ...
  resolve_from_cache_files <- function(remote, config, bioccache) {
    platforms    <- config$platforms
    rversions    <- config$`r-versions`
  ...
  make_bioc_resolution <- function(remote, platform, rversion, data,
