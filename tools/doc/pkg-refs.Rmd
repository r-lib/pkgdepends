...
- `reinstall` requests a re-install for package installations. It is
  supported by the `bioc::`, `cran::`, `git::`, `github::`, `local::`,
...
- `nocache` will ignore the package cache. It will always download the
  ...
  `github::`, `standard::` and `url::` remote types.
...
```
[<package>=]github::https?://<host>[<detail>]
```
...
Currently `r pak_or_pkgdepends()` only supports git servers that use
