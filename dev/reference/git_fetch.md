...
- blobs:
  ...
  Whether we want to download all blobs as well or not. We are still
  always requesting a shallow (depth = 1) clone.
...
- `object`: the object itself. It is a
  ...
  - a data frame for trees, with columns: `type`, `mode`, `path`,
    `hash`,
