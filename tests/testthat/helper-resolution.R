...
make_fake_resolution1 <- function(ref, args = list()) {
  ...
  def <- list(
    ...
    sources = c(
      ...
      sprintf("%s/%s/Archive/%s/%s", mirror, repodir, pref$package, filename)
    ),
...
make_fake_resolution <- function(...) {
  ...
  ress <- lapply_with_names(
    names(pkgs),
    function(n) make_fake_resolution1(n, pkgs[[n]])
  )
...
describe_fake_error <- function(
  ...
  solution <- list(status = "FAILED", data = pkgs, problem = lp, solution = sol)
