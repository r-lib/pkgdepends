...
remotes__download_github <- function(self, private, resolution) {
  ...
  cached_target <- file.path(cache_dir, files$target)
  cached_zip <- sub("\\.tar\\.gz$", ".zip", cached_target)
  mkdirp(target_dir <- dirname(target_file))
  mkdirp(dirname(cached_target))
  subdir <- resolution$remote$subdir
  url <- files$source

  if (is_valid_package(target_file) || is_valid_package(cached_target)) {
    ...
    build_github_package(cached_zip, cached_target, subdir)
