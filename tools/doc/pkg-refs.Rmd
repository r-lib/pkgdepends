...
Most of these functions can also take more generic _package references_
...
So `mypkg` is equivalent to `standard::mypkg` and `r pak_or_pkgdepends()`
...
`options("repos")`), then `r pak_or_pkgdepends()` uses the CRAN and
...
- a valid package name, or a package name with an `@`
  version specification, the `standard` package source is used. E.g. `pkg` is
  equivalent to `standard::pkg` and `pkg@1.0` is equivalent to
  `standard::pkg@1.0`.
- a valid `github` ref type without the `github::` prefix,
...
- a GitHub URL (see below) without the `github::` prefix,
  then `github` is used.
- a path that starts with `.` or `/` or `\` or `~`, then
...
- `ignore-before-r` is a version number parameter. The package will be
  ...
  `packgename=?ignore-before-r` form.
...
- `<subdir>` specifies an optional subdirectory, if the package is within a
...
- `<package>` is the name of the package. If this is missing, then
  the name of the repository is used.
- `<project-path>` is typically the GitLab user or group name, but
