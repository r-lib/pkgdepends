...
- ref:
  ...
  Either a SHA or a ref name. It may also be a branch name without the
  `refs/heads` prefix, or a partial (but unique) SHA of at least seven
  hexadecimal digits. See
  [`git_list_refs()`](https://r-lib.github.io/pkgdepends/dev/reference/git_list_refs.md)
  for how branches, tags and GitHub pull requests are named.
...
- `commit`: named character vector of the data of the commit object
  belonging to `ref`. It has fields named: `tree`, `parent`, `author`,
  `committer`, `encoding`, `message`.
